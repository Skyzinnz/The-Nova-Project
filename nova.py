...
Bitwise:     &  |  ^  <<  >>  ~
  ...
  var b  = 0b1010 & 0b1100     # bitwise AND -> 8
  var s  = 1 << 3              # left shift   -> 8
...
List Comprehensions
-------------------
Build a list from another list in one line.
...
Examples:
  ...
  say(f"Status: {"on" if active else "off"}")
""",
...
Fire an event (runs all registered handlers):
  emit <event>
  emit <event>(<arg1>, <arg2>)

Inside the handler, arguments are in __args__:
  on data(
    say(__args__)
  )
  emit data("hello", 42)

Events are useful for decoupled, reactive code.
""",
...
Run a block of code in a background thread.
  ...
  bg(
    wait 2
    say("This ran in the background!")
  )

Notes:
- The main program keeps running immediately.
- Background threads share the global scope.
- Use wait to introduce delays inside bg blocks.
- Useful for timers, listeners, and async polling.

Example - background timer:
  bg(
    wait 5
    say("5 seconds passed!")
  )
  say("Timer started, continuing...")
""",
...
Open a port:
  ...
  serial.open("COM3", 115200)          # Windows

Write data:
  serial.write("/dev/ttyUSB0", "hello")

Read data:
  var data = serial.read("/dev/ttyUSB0", 64)
  var line = serial.readline("/dev/ttyUSB0")

Check bytes available:
  var n = serial.available("/dev/ttyUSB0")
...
Full Arduino example:
  ...
  var resp = serial.readline("/dev/ttyUSB0")
  say(f"Arduino replied: {resp}")
  serial.close("/dev/ttyUSB0")
""",
}
